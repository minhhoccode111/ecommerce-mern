  ...
  static handleFilePath(field) {
    return (req, res, next) => {
      if (typeof field === "string") {
        ...
        if (req?.file?.path) {
          req.body[field] =
            "/" + StringUtils.replaceAll(req?.file?.path, "\\", "/");
        }
        ...
        for (let i = 0; i < field.length; i++) {
          ...
          if (req?.files?.[f.name] && req?.files?.[f.name].length > 0) {
            req.body[f.name] = req?.files?.[f.name].map(
              (x) => "/" + StringUtils.replaceAll(x.path, "\\", "/"),
            );
          }
        }
      }
  ...
  static clearUploadFile(req) {
    ...
    if (typeof field === "string") {
      ...
      for (let i = 0; i < field.length; i++) {
        ...
        if (
          req.body[f.name] &&
          req.body[f.name].length > 0 &&
          Array.isArray(req.body[f.name])
        ) {

...
const SELECTED_FIELDS =
  "_id numericId customer user address isReceiveAtStore status paymentMethod paymentStatus items subTotal shippingFee discount total createdAt updatedAt";
...
async function createWithTransaction(orderData, createdBy) {
  ...
  try {
    ...
    if (orderData.discountCode) {
      try {
        const { amount, info: discountInfo } =
          await discountService.calculateDiscountAmt(
    ...
    if (subTotal > 500000 || orderData?.isReceiveAtStore) {
      ...
      orderData?.address?.province?.toUpperCase().includes("HỒ CHÍ MINH")
    ) {
      shippingFee = 20000;
    } else {
      shippingFee = 30000;
...
async function tryCreate(customerInfo, orderData, createdBy) {
  ...
  while (countError < 3) {
    try {
      ...
      if (
        errMsg.startsWith("transaction") &&
        errMsg.endsWith("has been committed.")
      ) {
        countError++;
      } else {
        throw err;
...
async function create(customerInfo, orderData, createdBy) {
  if (mongoose.Types.ObjectId.isValid(customerInfo)) {
    const user = await User.findOne({ _id: customerInfo }, { _id: 1 })
      .lean()
      .exec();
    if (!user) {
  ...
  if (
    !Object.values(constants.ORDER.PAYMENT_METHOD).includes(

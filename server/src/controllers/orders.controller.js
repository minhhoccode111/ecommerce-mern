...
export const getOne = async (req, res, next) => {
  try {
    ...
    if (order) {
      ...
      ResponseUtils.status404(res, "Order not found");
    }
  } catch (err) {
    next(err);
  }
...
export const getList = async (req, res, next) => {
  ...
  try {
    ...
    let result;
    if (userId) {
      result = await orderService.getListByUser(
        userId,
        search,
        status,
        paymentStatus,
      ...
      if (!accessToken) {
        ResponseUtils.status401(res, "Access token is required");
        return;
      }

      const decodeData = await firebaseService.verifyToken(accessToken);
      if (!decodeData) {
        ResponseUtils.status401(res, "Access token is invalid");
        return;
      }

      if (decodeData.exp < Date.now() / 1000) {
        ResponseUtils.status401(res, "Access token is expired");
        return;
      }
    ...
    ResponseUtils.status200(res, "Get order list success", result);
  } catch (err) {
    next(err);
  }
};
...
export const getByUser = async (req, res, next) => {
  try {
    ...
    ResponseUtils.status200(res, "Get list order success", orders);
  } catch (err) {
    next(err);
  }
};
...
export const createByUser = async (req, res, next) => {
  try {
    ...
    if (order) {
      ...
      ResponseUtils.status201(res, "Create order success", order, {
        paymentUrl,
      });
    } else {
      ResponseUtils.status400(res, "Create order fail");
    }
  } catch (err) {
    next(err);
  }
...
export const rePayOrder = async (req, res, next) => {
  try {
    ...
    const order = await orderService.getOne(orderId);
    if (!order) {
      ResponseUtils.status400(res, "Order not found");
    } else if (order.paymentStatus === constants.ORDER.PAYMENT_STATUS.PAID) {
      ResponseUtils.status400(res, "Order has been paid");
      ...
      ResponseUtils.status200(res, "Create payment url success", paymentUrl);
    }
  } catch (err) {
    next(err);
  }
...
export const updateByUser = async (req, res, next) => {
  try {
    ...
    const order = await orderService.updateByUserId(
      req.user._id,
      orderId,
      req.body,
    );
    if (order) {
      ResponseUtils.status200(res, "Update order success", order);
    } else {
      ResponseUtils.status400(res, "Update order fail");
    }
  } catch (err) {
    next(err);
  }
...
export const updateOrderByAdminOrStaff = async (req, res, next) => {
  try {
    ...
    if (order) {
      ...
      ResponseUtils.status200(res, "Update order success", result);
    } else {
      ResponseUtils.status400(res, "Update order fail");
    }
  } catch (err) {

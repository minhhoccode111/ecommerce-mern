...
export const updateItemQty = async (req, res, next) => {
  try {
    ...
    const updateResult = await cartService.updateItemQty(
      userId,
      productId,
      sku,
      delta,
    ...
    if (updateResult) {
      ResponseUtils.status200(
        res,
        "Update item qty on cart successfully",
        updateResult,
      );
    } else {
...
export const deleteItem = async (req, res, next) => {
  try {
    ...
    if (result) {
      ResponseUtils.status200(
        res,
        "Remove item from cart cart successfully",
        result,
      );
    } else {
      ResponseUtils.status200(
        res,
        "Has error when remove item from cart",
        result,
      );
    }
  } catch (err) {

  ...
  try {
    ...
    if (discounts && discounts.length > 0) {
      ResponseUtils.status200(
        res,
        "Gets all discounts successfully",
        discounts,
      );
    } else {
...
export const getDiscount = async (req, res, next) => {
  try {
    ...
    if (discount) {
      ResponseUtils.status200(
        res,
        `Get discount '${discount.name}' successfully!`,
        formatDiscount(discount, req),
      );
    } else {
...
export const checkExistedCode = async (req, res, next) => {
  try {
    ...
    const found = await discountService.getAll({
      fields: "_id",
      filters: { code: code },
      isShowHidden: true,
      isShowAllDate: true,
...
export const estDiscountAmt = async (req, res, next) => {
  try {
    ...
    const result = await discountService.calculateDiscountAmt(
      code,
      orderSubtotal,
    );
...
export const deleteDiscount = async (req, res, next) => {
  try {
    ...
    if (result) {
      ResponseUtils.status200(
        res,
        `Deleted discount '${result.name}' successfully!`,
        result,
      );
    } else {
      ResponseUtils.status404(res, `Discount '${identity}' not found!`);
